...
const likeTweet = async (req, res, next) => {
  try {
    ...
    if (isLiked) {
      ...
      return res.status(200).json({ message: "Tweet unliked successfully" , likes : updatedTweet.likes });
    ...
    res.status(200).json({ message: "Tweet liked successfully" , likes : updatedTweet.likes } );
  } catch (error) {
...
const getUserTweets = async (req, res, next) => {
  try {
    ...
    console.error("Error fetching user tweets:", error);
    next(new CustomError("Error fetching user tweets", 500));
  }
};
