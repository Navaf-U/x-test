...
import {
  fetchUserData,
  updateUserProfile,
} from "@/lib/store/thunks/user-thunks";
...
const EditProfileModal: React.FC<EditProfileProps> = ({ user, onClose }) => {
  ...
  const [previewImage, setPreviewImage] = useState<string>(user.pfp || "");
...
const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (file) {
    ...
    try {
      await axiosInstance.post("/api/user/update/pfp", formData);
    } catch (error) {
  ...
  const handleBgImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      formData.append("file", file);
      try {
        await axiosInstance.post("/api/user/update/header", formData);
      } catch (error) {
